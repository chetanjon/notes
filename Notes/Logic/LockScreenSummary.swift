    ...
    enum Model {
        ...
        static func line(for text: String) async throws -> String {
            let session = LanguageModelSession(instructions: """
                The user gives you a note. Reply with one line of at most twelve words that says \
                what the note is about, in the language it is written in, with no full stop. It \
                will sit under the note's title on a Lock Screen, so it must stand on its own.
                """)
